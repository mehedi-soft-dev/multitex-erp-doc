...
front-matter embedded in docs/features/*.md and docs/schema/*.md.
...
feature or schema doc. See docs/CONTRIBUTING.md section 5-6.
...
SKIP_FILES = {"_template.md", "_conventions.md"}
...
def harvest_feature(path: Path, fields: dict) -> tuple[list[dict], list[dict]]:
    ...
    if service:
        ...
        nodes.append({"id": svc_id, "type": "Service", "source_file": str(path.relative_to(DOCS_ROOT.parent)), "name": service})
    ...
    if mvc:
        if looks_like_identifier(mvc.replace(".", "_")):
            mvc_id = f"mvc:{mvc}"
            nodes.append({"id": mvc_id, "type": "MvcAction", "source_file": str(path.relative_to(DOCS_ROOT.parent)), "handler": mvc})
...
def harvest_schema(path: Path, fields: dict) -> tuple[list[dict], list[dict]]:
    ...
    # Column-level FK edges. Encoding is deliberately a flat string, not nested YAML, to match
    # this file's hand-rolled flat-key:value parser. Two forms per entry:
    ...
    for col_entry in split_multi(fields.get("columns")):
        ...
        nodes.append({"id": col_id, "type": "Column", "source_file": str(path.relative_to(DOCS_ROOT.parent)), "table": tbl_id})
...
def main() -> int:
    ...
    print(f"wrote {len(all_nodes)} nodes -> {nodes_path.relative_to(DOCS_ROOT.parent)}")
    print(f"wrote {len(all_edges)} edges -> {edges_path.relative_to(DOCS_ROOT.parent)}")
    for w in WARNINGS:
